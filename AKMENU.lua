...
-- ═══════════════════════════════════════════════════
local Conns       = {}
local ESPDrawings = {}   -- [player] = {boxLines, skelLines, tracer, name, hp, dist}
...
local function ClearAllESP()
    for _, d in pairs(ESPDrawings) do
        if d.boxLines then for _, l in ipairs(d.boxLines) do KillDrawing(l) end end
        if d.skelLines then for _, l in ipairs(d.skelLines) do KillDrawing(l) end end
        KillDrawing(d.tracer); KillDrawing(d.name); KillDrawing(d.hp); KillDrawing(d.dist)
    end
    ESPDrawings = {}
...
local function Stk(p,col,t) local s=Instance.new("UIStroke"); s.Color=col or Color3.fromRGB(38,38,38); s.Thickness=t or 1; s.Parent=p; return s end
...
local KeyCard = N("Frame",{
    ...
    Position=UDim2.fromScale(.5,.5), BackgroundColor3=Color3.fromRGB(16,16,16),
...
N("TextLabel",{
    ...
    TextColor3=Config.Text, Font=Enum.Font.GothamBold, TextSize=18, ZIndex=3002,
},KeyCard)
...
local KeyErr = N("TextLabel",{
    ...
    TextColor3=Color3.fromRGB(215,55,55), Font=Enum.Font.Gotham, TextSize=10,
...
local function TryKey(k)
    ...
    if VALID_KEYS[k] then
        local tw = TweenService:Create(KeyScreen,TweenInfo.new(0.3),{BackgroundTransparency=1})
        tw:Play()
        tw.Completed:Connect(function() pcall(function() KeyScreen:Destroy() end) end)
    else
        ...
        task.spawn(function()
            for _, ox in ipairs({-6,6,-4,4,-2,2,0}) do shake(ox); task.wait(0.05) end
...
KeySubmit.MouseButton1Click:Connect(function() TryKey(KeyInput.Text) end)
...
local MF = N("Frame",{
    ...
    BackgroundColor3=Config.Primary, BorderSizePixel=0, ClipsDescendants=true,
},SG)
Cor(MF,12); Stk(MF,Color3.fromRGB(30,30,30),1)

-- Title bar
local TB = N("Frame",{
    Size=UDim2.new(1,0,0,44), BackgroundColor3=Config.Secondary, BorderSizePixel=0,
},MF)
Cor(TB,12)
...
local CL = Instance.new("UIListLayout"); CL.SortOrder=Enum.SortOrder.LayoutOrder; CL.Padding=UDim.new(0,3); CL.Parent=CatList
local CP = Instance.new("UIPadding"); CP.PaddingLeft=UDim.new(0,8); CP.PaddingRight=UDim.new(0,8); CP.PaddingTop=UDim.new(0,4); CP.Parent=CatList
...
local function MakePanel(name)
    ...
    local ll=Instance.new("UIListLayout"); ll.SortOrder=Enum.SortOrder.LayoutOrder; ll.Padding=UDim.new(0,6); ll.Parent=scroll
    local pp=Instance.new("UIPadding"); pp.PaddingLeft=UDim.new(0,12); pp.PaddingRight=UDim.new(0,14)
    pp.PaddingTop=UDim.new(0,10); pp.PaddingBottom=UDim.new(0,14); pp.Parent=scroll
    return scroll
end
...
local function Card(parent, h)
    local f=N("Frame",{
        Size=UDim2.new(1,0,0,h or 50), BackgroundColor3=Config.Secondary, BorderSizePixel=0,
    },parent)
    Cor(f,8); return f
end
...
local function Toggle(parent, opts)
    --[[opts: label, description, default, onToggle(on, sliderVal), slider={min,max,default,onChange}]]
    local hasSlider = opts.slider ~= nil
    local isOn = opts.default or false

    local function CardH() return hasSlider and (isOn and 82 or 50) or 50 end
    local card = Card(parent, CardH())

    local pill = N("TextButton",{
        Size=UDim2.new(0,40,0,22), Position=UDim2.new(1,-50,.5,-11),
        BackgroundColor3=isOn and Color3.fromRGB(52,172,90) or Color3.fromRGB(38,38,38),
    ...
    if hasSlider then
        ...
        local fill = N("Frame",{Size=UDim2.new(pct,0,1,0),BackgroundColor3=Config.Accent,BorderSizePixel=0},track)
        Cor(fill,2)
        local knob = N("Frame",{
        ...
        local function mv(x)
            ...
            curSVal=v; fill.Size=UDim2.new(rel,0,1,0); knob.Position=UDim2.new(rel,0,.5,0); valL.Text=tostring(v)
            if s.onChange then s.onChange(v) end
        end
        track.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then sdrag=true;mv(i.Position.X) end end)
        Track(UIS.InputChanged:Connect(function(i) if sdrag and i.UserInputType==Enum.UserInputType.MouseMovement then mv(i.Position.X) end end))
        Track(UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then sdrag=false end end))
    end

    pill.MouseButton1Click:Connect(function()
        isOn=not isOn
        TweenService:Create(pill,TweenInfo.new(0.15),{BackgroundColor3=isOn and Color3.fromRGB(52,172,90) or Color3.fromRGB(38,38,38)}):Play()
        TweenService:Create(dot,TweenInfo.new(0.15),{Position=isOn and UDim2.new(1,-19,.5,-8) or UDim2.new(0,3,.5,-8)}):Play()
        if sliderFrame then sliderFrame.Visible=isOn; card.Size=UDim2.new(1,0,0,CardH()) end
...
local function Slider(parent, label, mn, mx, def, onChange)
    ...
    local track=N("Frame",{Size=UDim2.new(1,-56,0,4),Position=UDim2.new(0,12,0,34),
        BackgroundColor3=Color3.fromRGB(36,36,36),BorderSizePixel=0},card); Cor(track,2)
    local pct=math.clamp((def-mn)/math.max(1,mx-mn),0,1)
    local fill=N("Frame",{Size=UDim2.new(pct,0,1,0),BackgroundColor3=Config.Accent,BorderSizePixel=0},track); Cor(fill,2)
    local knob=N("Frame",{Size=UDim2.new(0,13,0,13),AnchorPoint=Vector2.new(.5,.5),
        Position=UDim2.new(pct,0,.5,0),BackgroundColor3=Color3.fromRGB(232,232,232),BorderSizePixel=0,ZIndex=3},track); Cor(knob,7)
    local vL=N("TextLabel",{Size=UDim2.new(0,40,0,18),Position=UDim2.new(1,-50,0,8),
    ...
    track.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then d=true;mv(i.Position.X) end end)
    Track(UIS.InputChanged:Connect(function(i) if d and i.UserInputType==Enum.UserInputType.MouseMovement then mv(i.Position.X) end end))
    Track(UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then d=false end end))
    return card
end
...
local function Dropdown(parent, label, options, def, onChange)
    ...
    local btn=N("TextButton",{Size=UDim2.new(0,128,0,28),Position=UDim2.new(1,-140,.5,-14),
        BackgroundColor3=Color3.fromRGB(28,28,28),Text=def.." ▾",
        TextColor3=Config.Text,Font=Enum.Font.Gotham,TextSize=11,BorderSizePixel=0},card); Cor(btn,6)
    local list=N("Frame",{Size=UDim2.new(0,128,0,#options*28),Position=UDim2.new(1,-140,1,4),
    ...
    local ll2=Instance.new("UIListLayout"); ll2.SortOrder=Enum.SortOrder.LayoutOrder; ll2.Parent=list
    for _, opt in ipairs(options) do
        local ob=N("TextButton",{Size=UDim2.new(1,0,0,28),BackgroundTransparency=1,
        ...
        ob.MouseLeave:Connect(function() ob.TextColor3=ob.Text==sel and Config.Accent or Config.Text end)
...
local function Btn(parent, label, onClick, color)
    ...
    local b=N("TextButton",{Size=UDim2.fromScale(1,1),BackgroundTransparency=1,
...
-- Speed (bypass: BodyVelocity instead of WalkSpeed so WalkSpeed stays at 16)
local function BuildSpeedBV()
    ...
    bv.Name="__spd"; bv.MaxForce=Vector3.new(9e4,0,9e4); bv.Velocity=Vector3.zero; bv.Parent=hrp
    SpeedBV=bv
end
...
Toggle(PlayerP,{
    label="Speed", description="Enhanced movement (anti-cheat safe via BodyVelocity)",
    default=false,
...
local function DoFly()
    ...
    if FlyBody then pcall(function()FlyBody.bv:Destroy()end); pcall(function()FlyBody.bg:Destroy()end) end
...
local function StopFly()
    if FlyBody then pcall(function()FlyBody.bv:Destroy()end);pcall(function()FlyBody.bg:Destroy()end);FlyBody=nil end
    pcall(function()
...
do
    ...
    local pill=N("TextButton",{Size=UDim2.new(0,40,0,22),Position=UDim2.new(1,-50,.5,-11),
        BackgroundColor3=Color3.fromRGB(38,38,38),Text="",BorderSizePixel=0},card); Cor(pill,11)
    local dot=N("Frame",{Size=UDim2.new(0,16,0,16),Position=UDim2.new(0,3,.5,-8),
        BackgroundColor3=Color3.fromRGB(255,255,255),BorderSizePixel=0},pill); Cor(dot,8)
    ...
    local optF=N("Frame",{Size=UDim2.new(1,-24,0,28),Position=UDim2.new(0,12,0,52),
        BackgroundTransparency=1,Visible=false},card)
    local ol=Instance.new("UIListLayout");ol.FillDirection=Enum.FillDirection.Horizontal;ol.Padding=UDim.new(0,8);ol.Parent=optF

    local function JBtn(txt)
        local b=N("TextButton",{Size=UDim2.new(0,112,1,0),
            BackgroundColor3=S.JumpType==txt and Color3.fromRGB(40,40,40) or Color3.fromRGB(26,26,26),
            Text=txt,TextColor3=S.JumpType==txt and Config.Accent or Config.Text,
            Font=Enum.Font.Gotham,TextSize=10,BorderSizePixel=0},optF); Cor(b,6); return b
    end
    ...
    local function RefJ()
        ...
        dbl.BackgroundColor3=S.JumpType=="Double"   and Color3.fromRGB(40,40,40) or Color3.fromRGB(26,26,26)
        inf.BackgroundColor3=S.JumpType=="Infinity" and Color3.fromRGB(40,40,40) or Color3.fromRGB(26,26,26)
    end
    dbl.MouseButton1Click:Connect(function() S.JumpType="Double";   RefJ() end)
    inf.MouseButton1Click:Connect(function() S.JumpType="Infinity"; RefJ() end)

    local function ClearJ() for _,c in ipairs(jConns) do pcall(function()c:Disconnect()end) end; jConns={} end
    ...
    local function ApplyJ()
        ...
        if S.JumpType=="Double" then
            ...
            table.insert(jConns, hum.StateChanged:Connect(function(_,new)
                if new==Enum.HumanoidStateType.Landed or new==Enum.HumanoidStateType.Running then landed=true end
...
-- Noclip (improved pattern — skip HumanoidRootPart occasionally for less detection)
...
Track(RunService.Heartbeat:Connect(function()
    ...
    if S.FlyOn and FlyBody and FlyBody.bv and FlyBody.bv.Parent then
        ...
        FlyBody.bv.Velocity=(dir.Magnitude>0 and dir.Unit or Vector3.zero)*S.FlySpeed
    ...
    if S.KillAura and hrp then
        for _,p in ipairs(Players:GetPlayers()) do
            if p~=LP and p.Character then
                ...
                if eh and eh2 and eh2.Health>0 and (eh.Position-hrp.Position).Magnitude<=S.KillRange then
    ...
    if S.FastAtk and hum then
        pcall(function()
            ...
            for _,t in ipairs(anim:GetPlayingAnimationTracks()) do
                ...
                if n:find("attack") or n:find("swing") or n:find("punch") or n:find("slash") or n:find("hit") then
            ...
            for _,holder in ipairs({char, LP.Backpack}) do
                ...
                for _,v in ipairs(holder:GetDescendants()) do
                    if (v:IsA("IntValue") or v:IsA("NumberValue")) then
                        ...
                        if (n:find("ammo") or n:find("arrow") or n:find("bullet") or n:find("charge")) and v.Value<999 then
                            v.Value=999
                        end
                    end
                end
...
local function NewLine(col,thick)
    ...
    local l=Drawing.new("Line"); l.Color=col or Color3.fromRGB(255,55,55); l.Thickness=thick or 1.5; l.Visible=false; return l
end
local function NewTxt(txt,sz,col)
    if not hasDrawing then return nil end
    local t=Drawing.new("Text"); t.Text=txt or ""; t.Size=sz or 13; t.Color=col or Color3.fromRGB(255,255,255)
    t.Outline=true; t.OutlineColor=Color3.fromRGB(0,0,0); t.Visible=false; return t
end
...
local function InitESP(plr)
    ...
    for i=1,4  do table.insert(d.boxLines,  NewLine(Color3.fromRGB(255,60,60),1.5)) end
    for i=1,18 do table.insert(d.skelLines, NewLine(Color3.fromRGB(60,200,255),1))  end
    ESPDrawings[plr]=d
end
...
for _,p in ipairs(Players:GetPlayers()) do if p~=LP then InitESP(p) end end
...
Track(RunService.RenderStepped:Connect(function()
    ...
    local anyDraw=S.BoxESP or S.SkelESP or S.Tracer or S.HPBar or S.DistESP or S.NameESP
    ...
    for _,plr in ipairs(Players:GetPlayers()) do
        ...
        if S.Chams and not Highlights[plr] and plr.Character then UpdateChams(plr) end

        if not anyDraw then HideESP(plr); continue end
        local c=plr.Character; if not c then HideESP(plr); continue end
        local hrp=c:FindFirstChild("HumanoidRootPart"); if not hrp then HideESP(plr); continue end
        ...
        if S.BoxESP and hasDrawing then
            ...
            local sMin=Vector2.new(math.huge,math.huge); local sMax=Vector2.new(-math.huge,-math.huge)
            local allV=true
            ...
            else for _,l in ipairs(d.boxLines) do if l then l.Visible=false end end end
        else for _,l in ipairs(d.boxLines) do if l then l.Visible=false end end end
            ...
            for _,pair in ipairs(JOINTS) do
                ...
                if a and b and a:IsA("BasePart") and b:IsA("BasePart") then
                    ...
                    if sl then
                        ...
                        if va and vb then
                            sl.From=Vector2.new(sa.X,sa.Y); sl.To=Vector2.new(sb.X,sb.Y); sl.Visible=true
                        else sl.Visible=false end
                    end
                end
            end
            for i=idx+1,#d.skelLines do if d.skelLines[i] then d.skelLines[i].Visible=false end end
        else for _,l in ipairs(d.skelLines) do if l then l.Visible=false end end end

        -- TRACER
        if S.Tracer and d.tracer then
            d.tracer.From=Vector2.new(vc.X*.5,vc.Y); d.tracer.To=sp; d.tracer.Visible=true
...
for _,t in ipairs({
    ...
    {k="Tracer",  l="Tracer Lines",    d="Line from screen bottom to each player"},
    ...
    local k=t.k
    Toggle(VisualsP,{label=t.l,description=t.d,default=false,onToggle=function(on) S[k]=on end})
end

Toggle(VisualsP,{
    label="Chams / Highlight",
    description="Colour players with Highlight — always visible through walls",
    default=false,
    onToggle=function(on)
        S.Chams=on
        for _,p in ipairs(Players:GetPlayers()) do if p~=LP then UpdateChams(p) end end
    end
})
    ...
    onToggle=function(on)
        ...
        for _,p in ipairs(workspace:GetDescendants()) do
            if p:IsA("BasePart") then
                ...
                for _,plr in ipairs(Players:GetPlayers()) do
                    if plr.Character and p:IsDescendantOf(plr.Character) then isPlayerChar=true;break end
                end
                if not isPlayerChar then p.LocalTransparencyModifier=on and 0.72 or 0 end
            end
        end
    end
})
...
Toggle(CombatP,{label="Kill Aura",description="Auto-damage all players within radius",
    default=false,onToggle=function(on,val) S.KillAura=on;if val then S.KillRange=val end end,
    slider={min=1,max=100,default=20,onChange=function(v) S.KillRange=v end}})
Toggle(CombatP,{label="Reach / Hitbox Expander",description="Increase effective melee range",
    default=false,onToggle=function(on,val) S.Reach=on;if val then S.ReachVal=val end end,
    slider={min=1,max=60,default=10,onChange=function(v) S.ReachVal=v end}})
Toggle(CombatP,{label="Auto-Swing",description="Continuously attack at max rate",
    default=false,onToggle=function(on) S.AutoClick=on end})
Toggle(CombatP,{label="Fast Attack",description="3× animation speed on attack tracks",
    default=false,onToggle=function(on) S.FastAtk=on end})

SecLbl(CombatP,"CHARACTER BUFFS")
Toggle(CombatP,{label="No Recoil / No Spread",description="Eliminate weapon recoil and bullet spread",
    default=false,onToggle=function(on) S.NoRecoil=on end})
Toggle(CombatP,{label="Infinite Ammo / Arrows",description="Prevent ammo and arrow depletion",
    default=false,onToggle=function(on) S.InfAmmo=on end})
Toggle(CombatP,{
    label="God Mode",
    description="Instantly restore health — bypasses weak server-side validation",
    default=false,
    onToggle=function(on)
        S.GodMode=on
        if on then
...
Toggle(CombatP,{label="Anti-Stun / Anti-Knockback",description="Instantly cancel ragdoll and stun states",
    default=false,onToggle=function(on) S.AntiStun=on end})

SecLbl(CombatP,"BOW  (Survival Game Style)")
Toggle(CombatP,{label="Arrow Trajectory Visualiser",description="Show predicted arrow arc in world",
    default=false,onToggle=function(on) S.BowTrail=on end})
Toggle(CombatP,{label="Infinite Arrows",description="Arrows never run out (same as Infinite Ammo)",
    default=false,onToggle=function(on) S.InfAmmo=on end})
Toggle(CombatP,{label="Bow Zoom",description="Increase FOV zoom when drawing bow",
    default=false,
    onToggle=function(on)
        S.BowZoom=on
        if on then
            Track(RunService.RenderStepped:Connect(function()
                ...
                pcall(function()
                    ...
                    if tool and (tool.Name:lower():find("bow") or tool.Name:lower():find("arrow")) then
...
Track(RunService.RenderStepped:Connect(function()
    ...
    pcall(function()
        ...
        if not tool or not (tool.Name:lower():find("bow") or tool.Name:lower():find("arrow")) then return end
...
Dropdown(GameP,"Server Hop Mode",{"Any Server","Low Population"},"Any Server",function(v) hopMode=v end)
...
-- ── KEYBINDS ──────────────────────────────────────
SecLbl(SettingsP,"KEYBINDS  (click a button, then press any key or mouse button)")
...
for _,def in ipairs(KeybindDefs) do
    ...
    local id=def.id
    local kBtn=N("TextButton",{Size=UDim2.new(0,118,0,28),Position=UDim2.new(1,-130,.5,-14),
    ...
    Track(UIS.InputBegan:Connect(function(inp,gp)
        ...
        if inp.KeyCode==Enum.KeyCode.Escape then
            listeningID=nil; kBtn.Text=KName(Keybinds[id]); kBtn.TextColor3=Config.Text; return
        end
        local bound=nil
...
do
    ...
    -- Target selector
    local selF=N("Frame",{Size=UDim2.new(1,-24,0,28),Position=UDim2.new(0,12,0,30),BackgroundTransparency=1},card)
    local sl2=Instance.new("UIListLayout");sl2.FillDirection=Enum.FillDirection.Horizontal;sl2.Padding=UDim.new(0,8);sl2.Parent=selF
    ...
    local function CSBtn(txt)
        local b=N("TextButton",{Size=UDim2.new(.5,-4,1,0),
            BackgroundColor3=txt==colorTarget and Color3.fromRGB(40,40,40) or Color3.fromRGB(26,26,26),
            Text=txt.." Color",TextColor3=txt==colorTarget and Config.Text or Config.Sub,
            Font=Enum.Font.Gotham,TextSize=11,BorderSizePixel=0},selF); Cor(b,6); return b
    end
    local primB=CSBtn("Primary"); local secB=CSBtn("Secondary")
    local function UpdCS()
        primB.BackgroundColor3=colorTarget=="Primary"   and Color3.fromRGB(40,40,40) or Color3.fromRGB(26,26,26)
        secB.BackgroundColor3 =colorTarget=="Secondary" and Color3.fromRGB(40,40,40) or Color3.fromRGB(26,26,26)
    ...
    primB.MouseButton1Click:Connect(function() colorTarget="Primary";   UpdCS() end)
    secB.MouseButton1Click:Connect(function()  colorTarget="Secondary"; UpdCS() end)
    ...
    --[[
        ...
        Implementation:
          ...
          3. valLayer Frame child of svBox: black, UIGradient top=transparent, bottom=opaque
    ...
    local hKnob=N("Frame",{
        ...
        Position=UDim2.new(0,0,.5,0), BackgroundColor3=Color3.fromRGB(255,255,255),
    ...
    local function HexStr(c2)
        return string.format("#%02X%02X%02X",math.floor(c2.R*255),math.floor(c2.G*255),math.floor(c2.B*255))
    end
    ...
    local function SetH(x)
        local r=math.clamp((x-hueBar.AbsolutePosition.X)/hueBar.AbsoluteSize.X,0,1)
    ...
    hueBar.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then hDrag=true;SetH(i.Position.X) end
    end)
    -- Allow clicking on svBox OR valOver (both map to same SV coords)
    svBox.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 then svDrag=true;SetSV(i.Position.X,i.Position.Y) end
    end)
...
do
    ...
    N("TextLabel",{Size=UDim2.new(1,-24,0,18),Position=UDim2.new(0,12,0,8),
        BackgroundTransparency=1,Text="Manual  (min "..MIN_W.."×"..MIN_H.."  max "..MAX_W.."×"..MAX_H..")",
        TextColor3=Config.Sub,Font=Enum.Font.Gotham,TextSize=10,
        TextXAlignment=Enum.TextXAlignment.Left},card)

    local function SBox(xOff,val)
        local tb=N("TextBox",{Size=UDim2.new(0,60,0,28),Position=UDim2.new(1,xOff,0,28),
